...
DEBoff.size <- function(Pars, y=x, offvar = 48.12555^2){ #lognormal distribution of offspring weights
...
#which(!c(300,Res_w$T) == c(Res_w$T,299))
#  starvx(17930, Pars=c(T = 285, kappa = 0.83, Y = 1))
...
plot(x,Vsurvfun(x),type="l", col="red")
GR_pars <- c(T = 292,     # parameters for Temperature, feeding, allocation and Mass dependence
             kappa = 0.83, # allocation to respiration (Growth and maintenance)
             Y = 1)         # Feeding level
lines(x,VsurvfunT(x, GR_pars),type="l", col="blue")
