...
The total sum here (-0.04404539) is very close to the sensitivity of lambda to $\kappa_0$. I compare the size dependent sensitivity from these calculations to the summed demographic contributions in a plot in point below.
...
# and the sensitivity from the kernel matrix:
lines(x,Sens_Kmat[1,2:(n+1)]+apply(Sens_Kmat[2:(n+1),],1,sum), lty=2, lwd=2) # here I add the sensitivity contribution from fecundity (first row) to the rest of the sensitivity
```

So I get the same results in my comparisons of both total and size dependent sensitivity with the dempgraphic function contributions to sensitivity. 
...
### 4 - Plot sensitivity contributions (figure 5  main text)
```{r, warning=FALSE}
...
s_sum_text <- data.frame(Size = c(1500,1500,1500,4000,4000,4000,6000,6000,6000), Sens = 0.075,
