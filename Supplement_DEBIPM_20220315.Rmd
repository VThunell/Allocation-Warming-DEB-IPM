...
S4. Sensitivity analysis of lambda to $\kappa_0$: Methods and analytical expression for derivatives of demographic functions to $\kappa_0$.

S5. Growth trajectories and fecundity at for optimal allocation ($\kappa_0^*$.)

S6. Fitness landscape & stable size structure for survival rate contrasts
...
for(i in 1:10){ 
  a[i,1] <- mean(sample(x = x, size = 1 , replace = TRUE, prob = age1size(test_Pars)))  
    for(j in 2:20){ # 20 years of growth
      a[i,j] <- mean(sample(x = x, size = 1 , replace = TRUE, prob = growthfun(a[i,j-1],test_Pars)))
  }
}
...
## S4. Sensitivity of the DEBIPM: Methods and analytical expression for derivatives of demographic functions to $\kappa_0$.
...
As all eggs transition to 1-year olds (or die) next year, the upper left element of the matrix is zero. The other parts of the matrix are defined by the DEBIPM, the discretized version assumes the model consists of $n+1$ stages, where the first stage is the egg stage and the last stages from 2 to $n+1$ are size stages. The term $g_{ij}$ is the probability of transitioning from size stage $j$ to size stage $i$ and are determined by the underlying DEB-function $g(m_{s+1};m_s,T)$. The  projection matrix is then given by
...
&=\int_0^{t_l}\alpha C(m,T)e^{-\frac{m}{18000\kappa_m}}dt.
\end{align}
```
...
```{=Latex}
\begin{align}\label{dmu.dkappa2}
\frac{d \mu(m_s)}{d\kappa_0} &=\frac{d}{d\kappa_0}\int_0^{t_l/2}[\kappa_0e^{-\frac{m}{18000\kappa_m}}\alpha C(m,T)-M(m,T)]dt\\
&=\int_0^{t_l/2}\alpha C(m,T)e^{-\frac{m}{18000\kappa_m}}dt.
\end{align}
```
...
f(m_s,T)&=\frac{1}{2e_m}\int_0^{t_l}\frac{dR}{dt}dt=\frac{1}{2e_m}\int_0^{t_l}(1-\kappa_0e^{-\frac{m}{18000\kappa_m}}\alpha C(m,T))dt.
\end{align}
```
...
Size dependent growth and fecundity for optimal allocation $(\kappa_0^*)$ differs from those presented in figure 3 in the main text (i.e. with $\kappa_0=0.8$ which was used to fit the model). Optimal allocation in our Windermere pike DEBIPM (main text figure 2) results in increased growth (mean size at age, Fig. S5A) but decreased size specific fecundity (Fig. S5B) compared to $\kappa_0=0.8$) of energy to somatic growth. 
...
In the main analyses, we contrast the size- and temperature dependent survival used in our model using two alternative survival scenarios (Fig. 4C main text). Here we present the corresponding fitness landscape, $\kappa_0^*$ (Fig S6A,B) and stable structure $w$ at the reference temperature (287 K, Fig S6C,D) and over temperature (Fig. S6E,F) for the temperature independent survival contrast $a(m_s)$ (Fig. S6A,C,E) and constant survival $a=0.68$ (Fig. S6B,D,F). In the first scenario, we use temperature independent survival a(m_s) that correspond to survival at mean temperature (287 K) in Windermere where temperature does not affect the survival asymptote for mass ($a(m_s)$, (Fig. 4D). In the second we use constant (both size- and temperature-independent) survival that equals the this asymptote at mean temperature (a=0.68, Fig. 4E).
...
Res_w_2 <- read.delim("//storage-og.slu.se/home$/vitl0001/My Documents/Manus2/Results/Results0224/Res_w_2_2022-02-24.txt", sep = ",")
...
FigS6e <- 
  ...
  geom_contour_filled(breaks = c(min(log(Res_w_2_long$biom)), 
    ...
    scale_fill_manual(values = mycolors, name="Relative densities", 
                    labels=c(paste("< ",signif(1e-10,2)), 
                             ...
                             paste(">",signif(max(Res_w_2_long$biom),2)))) + 
  ...
  annotate(geom="text", -Inf, Inf, label="E", hjust = -14, vjust = 2, size= 4, fontface = "bold") +
...
Res_w_3 <- read.delim("//storage-og.slu.se/home$/vitl0001/My Documents/Manus2/Results/Results0224/Res_w_3_2022-02-24.txt", sep = ",")
...
FigS6f <- 
  ...
  annotate(geom="text", -Inf, Inf, label="F", hjust = -15, vjust = 2, size= 4, fontface = "bold")+
  scale_fill_manual(values = mycolors, name="Relative densities", 
                    labels=c(paste("< ",signif(1e-10,2)), 
                             signif(seq(1e-10,1e-7,length.out = 5),2),
                             paste(">",signif(max(Res_w_3_long$biom),2)))) + 
  scale_y_continuous(name = "Temperature [K]", limits = c(284,294),breaks = seq(285,293,2))+
  scale_x_continuous( name = expression(paste("Mass ",italic("m"["s"])," [g]")), limits = c(0,20000))+
  coord_cartesian(ylim = c(284,294)) +
  theme_bw() +
...
Lindmark, M., J. Ohlberger, and A. Gårdmark. 2022. Optimum growth temperature declines with body size within fish species. Glob Chang Biol.
