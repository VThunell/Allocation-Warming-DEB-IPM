...
nL <-
as_tibble(nlam) %>%
  ggplot(., aes(nlam$n,lambda, color = as.factor(T))) +
  geom_line() +
  scale_color_manual(values = c('#4575b4','#91bfdb','#fee090','#fc8d59','#d73027'), name="Temp [K]")+
  theme_bw() +
...
Figure S6. Fitness landscape (A,B) with solid line representing the optimal allocation strategy ($\kappa_0^*$), and the stable structure $w$ for four temperatures (C,D) and over temperature (E,F) for survival contrast $a(m_s)$(A,C,E) and $a=0.68$ (B,D,F).
