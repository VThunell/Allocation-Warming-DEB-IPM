...
cIa=0
cId=0.01
c2 <- cbind(cId,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c2 <- rbind(c2,cbind(cId,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c2 <- rbind(c2,cbind(cId,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))
cId=0
c1 <- cbind(cId,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c1 <- rbind(c1,cbind(cId,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c1 <- rbind(c1,cbind(cId,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))
cId=-0.01
c0 <- cbind(cId,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c0 <- rbind(c0,cbind(cId,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c0 <- rbind(c0,cbind(cId,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))


A1_d1<- cbind(Temp = rep(275:299,9), rbind(c2,c1,c0))
...
cId = 0
cIa=-0.01
c2 <- cbind(cIa,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c2 <- rbind(c2,cbind(cIa,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c2 <- rbind(c2,cbind(cIa,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))
cIa=0
c1 <- cbind(cIa,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c1 <- rbind(c1,cbind(cIa,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c1 <- rbind(c1,cbind(cIa,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))
cIa=0.01
c0 <- cbind(cIa,Mass=1,Consumption = rI_T_GU2(280:299, 10), 
            Maintenance = rM_T_AL2(280:299, 10)) 
c0 <- rbind(c0,cbind(cIa,10,Consumption = rI_T_GU2(280:299, 100),
                     Maintenance = rM_T_AL2(280:299, 100)))
c0 <- rbind(c0,cbind(cIa,100,Consumption = rI_T_GU2(280:299, 1000),
                     Maintenance = rM_T_AL2(280:299, 1000)))


A1_d2<- cbind(Temp = rep(280:299,9), rbind(c2,c1,c0))
...
ggplot(A1_d1_long, aes(Temp, value, color = Rate, linetype = as.factor(Mass))) +
  ...
  scale_x_continuous("Temperature [K]", limits= c(280:299), breaks = c(280,284,288,292,296,300)) +
...
ggplot(A1_d2_long, aes(x=Temp, y = value, color = Rate, linetype = as.factor(Mass))) +
  ...
  scale_x_continuous("Temperature [K]", limits= c(280,299), breaks = c(280,284,288,292,296,300)) +
  ggtitle(expression(paste(italic("T")["0"]," = 292 Kelvin"))) +
  theme_bw() +
  theme(strip.background = element_rect(fill = "transparent", colour =  "transparent"),)
```
...
#                            alpha*GR_pars["Y"]*intake - maint, 
#                          repro_E = alpha*
# GR_pars["Y"]*(1-kap_fun(x,GR_pars["kappa"]))*intake))
#    }
# 
# T_rates0 <- NULL
# cIa <- 0.01
...
```{r, echo = FALSE, warning = FALSE}
T_rates_long %>%
  filter(as.factor(Temp) %in% c(287,288,289)) %>%
  ggplot(.) +
...
```{r, echo = FALSE, warning = FALSE}
T_rates_long %>%
  filter(as.factor(mass) %in% c(T_rates_long$mass[c(5,50,100,200)])) %>%
  ggplot(.) +
...
gro <- ggplot(trajcIa) +
  ...
  geom_line(data=trajcIa,size=1 , aes(t, mass, color = as.factor(Temp))) +
  #facet_wrap(~cIa) +#, labeller = label_both) +
  # ylim(0,0.012) + #ylim(0,NA) +
  # xlim(0,5) + #ylim(0,NA) +
  ylab("Weight [g]") +
...
xr<-sort(sample(x, 100, replace = FALSE))
rep<-NULL
for(i in Temp){
  repPh <- 
    as.tibble(cbind(xr,DEBrepfun(xr, c(T = i, kappa = kappp, 
                                       Y = 1)),i))
  colnames(repPh) <- c("mass","fec","Temp")
  rep <- rbind(rep, repPh)
}
x
repp <- ggplot() +
  geom_point(data=FData, aes(Weight, Eggs), alpha=0.1) +
  geom_line(data=rep, size=1, aes(mass, fec, color = as.factor(Temp))) +
