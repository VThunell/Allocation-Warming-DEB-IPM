...
el_surv = 5e-5 # egg & larvae survival. losely based on Kipling and Frost 1970 1/50000 from laid egg to age 2. 1.9e-4 in Vindenes 2014
...
DEBrepfun <- function(x, Pars, o_E) { # Note that intake refers to net energy available for growth, reproduction and maintenance
  ...
  fecund <- function(m, Pars, o_E) { 
    ...
    fe <- ifelse(f[,"mass",] < Devm,  0, #is it mature? # see text for the following conditions...
            ifelse(Pars[["Kappa"]]*f[,"intake",] >= f[,"maintenance",], f[,"intake",]*(1-Pars[["Kappa"]]), #can available energy from intake cover maintenance, NOTE:intake is already scaled with Y*alpha
              ifelse((Pars[["Kappa"]]*f[,"intake",] < f[,"maintenance",]) & (f[,"maintenance",] <= f[,"intake",]), 
