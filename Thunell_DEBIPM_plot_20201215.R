...
source("Thunell_DEBIPM_model_20201215.R")
...
plot(x,Kappa*alpha*Y*eps1*x^eps2, #Imax, 
...
for(i in Temp) {
  ...
  T_rates  <- rbind(T_rates,
                   cbind(Temp = i, mass = x, 
                         maint, intake, 
                         growth_E = Kappa*alpha*Y*intake - maint, 
                         repro_E = alpha*Y*(1-Kappa)*intake))
  }
...
GR_pars <- c(T =281,     # parameters for Temperature, feeding, allocation and Mass dependence
             Kappa = 0.6, # allocation to respiration (Growth and maintenance)
             Y=1)         # Feeding level
